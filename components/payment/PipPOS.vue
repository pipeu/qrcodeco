<template>
    <div>
        <p style="background-color:#FFFFCC; padding:5px; border:1px solid #EEE; margin-top:5px; font-size: 14px;">
            Nosso entregador levará a máquina de Cartão para o pagamento com TICKET, BEN, VeroCard, Função Débito ou Crédito
            <br>ps: Aceitamos Alimentação ou Refeição em todos os Tickets
        </p>

        <div style="margin:15px 0px 15px;" align="center">
            <PipButton @click="processPOS" :disabled="isProcessing" >Confirmar Pedido</PipButton>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            processPOS () {
                console.log('### payment.processPOS')
                let vm = this

                paymentApi.processPOS(vm.getCheckout(), vm.cpf,
                    vm.useInvoiceDetails, vm.chargeData).then((res) => {
                    console.log('paymentApi.processPOS response ', res.data)
