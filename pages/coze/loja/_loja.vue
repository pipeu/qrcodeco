...
export default {
  ...
  middleware ({ redirect }) {
    console.log('middleware loja redirecting to coze.com.br')
    return redirect('https://coze.com.br');
  },

  async asyncData ({ store, params, query, route, req, error, redirect }) {
    console.log('loja.vue async data params.pending:', params.pending)
    console.log('loja.vue async data => route.path:', route.path)
